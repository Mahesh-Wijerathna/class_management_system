...
const CashierDashboardSidebar = (permissions = []) => {
  return filterSidebarByPermissions(cashierSidebarSections, permissions);
};

export const useCashierSidebar = () => {
  const [sidebarSections, setSidebarSections] = useState(cashierSidebarSections);
      ...
      try {
        ...
        if (stored) {
          try {
            ...
            console.error('Failed to parse stored user data for cashier sidebar:', err);
          }
        }
        ...
        if (!userId) {
          setSidebarSections(cashierSidebarSections);
          return;
        }

        const permissions = await getUserPermissions(userId);
        setSidebarSections(CashierDashboardSidebar(permissions || []));
      } catch (error) {
        console.error('Failed to load cashier permissions:', error);
        setSidebarSections(cashierSidebarSections);
      } finally {
        setLoading(false);
      }
